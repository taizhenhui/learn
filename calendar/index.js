$(function () {
  ...
  function init() {
    ...
    // 初始化 年
    let minY = moment(minTime).format('YYYY'), maxY = moment(maxTime).format('YYYY');
    ...
    // 获取日历数组 初始化
    curCalendar = createDatas(minYear, minMonth)
    // 创建dom 初始化
  ...
  function setNewCurDate(date) {
    ...
    if (curYear === selectYears[1]) createOpt(setMonthDom, selectMonthsTwo)
  ...
  function createDatas(y, m) {
    let time = y + '-' + m,
      ...
      nextMonth = moment(time).month(moment(time).month() + 1).format("YYYY-MM"),
    ...
    // 获取剩余 天数的数组
    let arr = []
    for (let o = firstArr[firstArr.length - 1].day + 1; o <= curMonthDays; o++) {
  ...
  function createDom(curCalendar) {
    ...
    for (const tr of curCalendar) {
      ...
      for (const td of tr) {
        ...
        if (td.isDisabled) {
          let classNames = tdDom.prop('class') + ' disabled'
          tdDom.prop('class', classNames)
        }
        if (!td.isCurMonthDays) {
          let classNames = tdDom.prop('class') + ' disCurMonthDays'
          tdDom.prop('class', classNames)
        }
        trDom.prop('class', 'tab-tr')
        trDom.append(tdDom)
      }
  ...
  function createObjDate(date, isCurMonthDays) {
    let obj = {
      isSelected: 'no', // 是否被选中 yes 选中，no 未选中
